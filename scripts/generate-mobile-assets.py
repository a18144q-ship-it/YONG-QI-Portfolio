...
def convert(source: Path, destination: Path) -> None:
    ...
    with Image.open(source) as image:
        if getattr(image, "is_animated", False):
            frames = [resize(frame, 1800) for frame in ImageSequence.Iterator(image)]
        ...
        resized = resize(image, 3200)
        resized.save(destination, "WEBP", quality=95, method=6)
...
def main() -> None:
    ...
    for source in STATIC_ROOT.rglob("*"):
        ...
        if relative.parts[0] in {"assets", "mobile"} or relative.name in {"favicon.png", "og.png", "portfolio-ufo-cover.jpg"}:
            continue

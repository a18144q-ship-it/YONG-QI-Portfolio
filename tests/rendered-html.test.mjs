...
test("server-renders the complete portfolio case openings", async () => {
  ...
  assert.match(html, /data-mobile-preload-src="\/preview-mobile\/v2\/crossborder\/truck-roll-bar\/strategy-hero\.webp"/);
...
test("keeps the editorial opening system shared and ratio-safe", async () => {
  ...
  assert.match(page, /data-mobile-preload-src=\{mobilePreviewSrc\}/);
  assert.match(page, /media="\(max-width: 900px\)" srcSet=\{active \? currentMobileSrc/);
  assert.match(page, /data-full-src=\{src\}/);
  assert.match(page, /rootMargin: "1800px 0px"/);
  assert.match(page, /if \(!useOriginal\)/);
  assert.match(page, /const useMobilePreviews = window\.matchMedia\("\(max-width: 900px\)"\)\.matches/);
  assert.match(page, /const workerCount = useMobilePreviews \? 3 : 6/);
